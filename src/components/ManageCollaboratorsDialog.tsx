            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-[1fr_160px_auto]">
              <div className="relative">
                <select
                  ...
                  className="h-10 w-full appearance-none rounded-lg border border-slate-300 bg-white px-3 py-2 pr-9 text-sm text-slate-900 outline-none transition-all focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-1 disabled:cursor-not-allowed disabled:opacity-50"
                >
              ...
              <div className="relative">
                <select
                  value={selectedRole}
                  onChange={(event) => setSelectedRole(event.target.value as EditableListMemberRole)}
                  ...
                  <div className="flex items-center gap-2">
                    <div className="relative">
                      <select
                        value={member.role}
                        disabled={isLoading}
                        onChange={(event) => void handleRoleChange(member, event.target.value as EditableListMemberRole)}
                        className="h-9 appearance-none rounded-lg border border-slate-300 bg-white px-9 pr-8 text-sm text-slate-900 outline-none transition-all focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-1 disabled:cursor-not-allowed disabled:opacity-50"
                      >
                        {editableRoles.map((role) => (
                          <option key={role} value={role}>
                            {formatRole(role)}
                      ...
                      {member.role === "EDITOR" ? (
                        <PenLine className="pointer-events-none absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-slate-500" />
                      ) : (
                        <Eye className="pointer-events-none absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-slate-500" />
                      )}
